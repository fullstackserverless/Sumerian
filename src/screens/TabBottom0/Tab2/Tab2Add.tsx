...
const Tab2Add = ({ route, navigation }: Tab2AddT): ReactElement => {
  ...
  const createObj = async (values: ObjT) => {
    try {
      const obj = await DataStore.save(new ReactNative({ ...values }))
  ...
  const updateObj = async ({ title, description, img, uri }: ObjT) => {
    try {
      setLoading(true)
      const original = await DataStore.query(ReactNative, input.id)
      const update = await DataStore.save(
        ReactNative.copyOf(original, (updated) => {
          updated.title = title
  ...
  const deleteObj = async () => {
    try {
      setLoading(true)
      const obj = await DataStore.query(ReactNative, input.id)
