...
import { goBack, paleBlue, white, gray } from '../../../constants'
import { AppContainer, Input, Space, Button, Header, ButtonLink } from '../../../components'
import { RootStackParamList, ObjT } from '../../../AppNavigator'
import { createReactNative, updateReactNative, deleteReactNative } from '../../../graphql/mutations'

type ProfileScreenNavigationProp = StackNavigationProp<RootStackParamList, 'TAB2_ADD'>
type ProfileScreenRouteProp = RouteProp<RootStackParamList, 'TAB2_ADD'>
...
const Tab2Add = ({ route, navigation }: Tab2AddT): ReactElement => {
  ...
  const createObj = async (values: ObjT) => {
    setLoading(true)
    try {
      await API.graphql(graphqlOperation(createReactNative, { input: values }))
      goBack(navigation)()
      setLoading(false)
    } catch (err) {
      setError(err)
  ...
  const updateObj = async (values: ObjT) => {
    setLoading(true)
    try {
      await API.graphql(graphqlOperation(updateReactNative, { input: values }))
      goBack(navigation)()
      setLoading(false)
    } catch (err) {
      setError(err)
  ...
  const deleteObj = async ({ id }: ObjT) => {
    setLoading(true)
    try {
      await API.graphql(graphqlOperation(deleteReactNative, { input: { id } }))
      goBack(navigation)()
      setLoading(false)
    } catch (err) {
      setError(err)
  ...
  return (
    <AppContainer onPress={goBack(navigation)} loading={loading} color={paleBlue}>
      ...
      >
        {({ values, handleChange, errors, setFieldTouched, touched, handleSubmit }) => (
          <>
            ...
            />
            <Space height={40} />
            <Button title={check ? 'Update' : 'Create'} onPress={handleSubmit} color={paleBlue} />
            {check && (
              <>
                <Space height={10} />
                <ButtonLink title="or" textStyle={{ alignSelf: 'center' }} />
                <Space height={15} />
                <Button title="DELETE" onPress={() => deleteObj(values)} cancel color={paleBlue} />
              </>
            )}
          </>
        )}
