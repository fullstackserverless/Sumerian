...
const Tab4Add = ({ route, navigation }: Tab4AddT): ReactElement => {
  ...
  const [input, setJob] = useState<ObjT>({
    ...
    img: 'https://s3.eu-central-1.wasabisys.com/ghashtag/AWSForKids/DataStore.png',
    uri: 'rKTHKZ87Hh4',
    json: ''
  })
  ...
  const createObj = async (values: ObjT) => {
    setLoading(true)
    try {
      await API.graphql(graphqlOperation(createAmplify, { input: values }))
      goBack(navigation)()
      setLoading(false)
    } catch (err) {
      setError(err)
  ...
  const updateObj = async (values: ObjT) => {
    setLoading(true)
    try {
      await API.graphql(graphqlOperation(updateAmplify, { input: values }))
      goBack(navigation)()
      setLoading(false)
    } catch (err) {
      setError(err)
  ...
  const deleteObj = async ({ id }: ObjT) => {
    setLoading(true)
    try {
      await API.graphql(graphqlOperation(deleteAmplify, { input: { id } }))
      goBack(navigation)()
      setLoading(false)
    } catch (err) {
      setError(err)
  ...
  return (
    <AppContainer onPress={goBack(navigation)} loading={loading} color={white}>
      <Header onPress={goBack(navigation)} iconLeft="angle-dobule-left" colorLeft={fuchsia} />
      <Space height={70} />
      <Formik
        innerRef={formikRef}
        initialValues={input}
      ...
      >
        {({ values, handleChange, errors, setFieldTouched, touched, handleSubmit }) => (
          <>
            ...
            />
            <Space height={40} />
            <Button title={check ? 'Update' : 'Create'} onPress={handleSubmit} color={fuchsia} />
            {check && (
              <>
                <Space height={10} />
                ...
                <>
                  <Space height={15} />
                  <Button title="DELETE" onPress={() => deleteObj(values)} cancel color={fuchsia} />
                </>
              </>
            )}
          </>
